// cloud-control.ts (ES module style)

import Control from 'ol/control/Control';
import { unByKey } from 'ol/Observable';

// Possibly import the new OL event for rendering
// (modern OL might need 'postrender' instead of 'postcompose')
import MapEvent from 'ol/MapEvent';
import { Map } from 'ol';

export default class CloudControl extends Control {
  private _listener: any;
  private cloud: HTMLImageElement;
  private bird: HTMLImageElement;
  private particules: Array<any>;
  private birds: Array<any>;
  private wind: { angle: number; cos: number; sin: number; speed: number };
  ...
  constructor(options: any = {}) {
    ...
    // Initialize images from options or use defaults
    this.cloud = options.img || new Image();
    if (!options.img) {
      // base64 data or a fallback
    ...
    this.bird = options.bird || new Image();
    if (!options.bird) {
      // base64 data or a fallback
  ...
  public setWind(options: any = {}) {
    const rnd = Math.random;
    const a = options.windAngle || rnd() * Math.PI;
    const speed = options.windSpeed || rnd();
    this.wind = { angle: a, cos: Math.cos(a), sin: Math.sin(a), speed: speed };
  }

  // Called by OpenLayers when the control is added to or removed from a map
  override setMap(map: Map) {
    ...
    super.setMap(map);
    if (map) {
      this._listener = map.on('postrender', this.drawCloud_.bind(this));
    }
  }

  // Main animation logic
  private drawCloud_(event: MapEvent) {
    debugger;
    ...
    if (!p) {
      ...
      this.birds = [];
      for (i = 0; i < 5; i++) {
        var b = { angle: rnd() * 2 * Math.PI, x: rnd() * canvas.width, y: rnd() * canvas.height, rot: 0, fly: 0, cos: 0, sin: 0  };
